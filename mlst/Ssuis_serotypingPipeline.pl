...
my @commands;
if($SingleOrPaired eq "pe"){
	@commands = ("srst2 --input_pe $ss --forward $forward --reverse $reverse --output $scoresName\_MLST --log --mlst_db $MLST_input --mlst_definitions $MLST_definitions_file --save_scores", "srst2 --input_pe $ss --forward $forward --reverse $reverse --output $scoresName\_VirulenceFactors --log --gene_db $virulence_input --save_scores", "srst2 --input_pe $ss --forward $forward --reverse $reverse --output $scoresName --log --mlst_db $fasta_input --mlst_definitions $definitions_file --save_scores");
}
elsif($SingleOrPaired eq "se"){
	@commands = ("srst2 --input_se $ss --output $scoresName\_MLST --log --mlst_db $MLST_input --mlst_definitions $MLST_definitions_file --save_scores", "srst2 --input_se $ss --output $scoresName\_VirulenceFactors --log --gene_db $virulence_input --save_scores", "srst2 --input_se $ss --output $scoresName --log --mlst_db $fasta_input --mlst_definitions $definitions_file --save_scores");
}
...
system("/usr/bin/samtools faidx $gene_fasta");
...
foreach my $one (@furtherAnalysis_1){
	...
	system("/usr/bin/samtools sort $one\_vs_cpsK.bam -o $one\_vs_cpsK.sorted.bam");
    #system("/usr/bin/samtools mpileup -u -L 1000 -f $gene_fasta -Q 20 -q 1 $one\_vs_cpsK.sorted.bam > $one\_vs_cpsK.pileup");
    system("/usr/bin/bcftools mpileup -L 1000 -f $gene_fasta -Q 20 -q 1 $one\_vs_cpsK.sorted.bam > $one\_vs_cpsK.pileup");

    print "breakpoint 370";
    print "\n";
    my $bam=`ls -sh $one\_vs_cpsK.pileup`;
    print $bam, "\n";
    print "breakpoint 370\n";

    #system("/usr/bin/bcftools view -vcg $one\_vs_cpsK.pileup > $one\_vs_cpsK.raw.vcf");
    #system("/usr/bin/bcftools view $one\_vs_cpsK.pileup > $one\_vs_cpsK.raw.vcf");
    system("/usr/bin/bcftools call -mv -Ov $one\_vs_cpsK.pileup > $one\_vs_cpsK.raw.vcf");

	system("/usr/bin/vcfutils.pl varFilter -Q 20 $one\_vs_cpsK.raw.vcf > $one\_vs_cpsK.vcf");
...
foreach my $two (@furtherAnalysis_2){
	...
	#Run Samtools to call SNPs
	system("/usr/bin/samtools view -b -o $two\_vs_cpsK.bam -q 1 -S $two\_vs_cpsK.sam");

    print "breakpoint 443";
    print "\n";
    my $bam=`ls -sh $two\_vs_cpsK.bam`;
    print "$bam", "/n";

    #system("/usr/bin/samtools sort $two\_vs_cpsK.bam $two\_vs_cpsK.sorted");
	system("/usr/bin/samtools sort $two\_vs_cpsK.bam -o $two\_vs_cpsK.sorted.bam");

    #system("samtools mpileup -u -L 1000 -f $gene_fasta -Q 20 -q 1 $two\_vs_cpsK.sorted.bam > $two\_vs_cpsK.pileup");
	system("/usr/bin/bcftools mpileup -L 1000 -f $gene_fasta -Q 20 -q 1 $two\_vs_cpsK.sorted.bam > $two\_vs_cpsK.pileup");

    #system("/usr/bin/bcftools view -vcg $two\_vs_cpsK.pileup > $two\_vs_cpsK.raw.vcf");
    #system("/usr/bin/bcftools view $two\_vs_cpsK.pileup > $two\_vs_cpsK.raw.vcf");
	system("/usr/bin/bcftools call -mv -Ov $two\_vs_cpsK.pileup > $two\_vs_cpsK.raw.vcf");

	system("/usr/bin/vcfutils.pl varFilter -Q 20 $two\_vs_cpsK.raw.vcf > $two\_vs_cpsK.vcf");

    print "breakpoint 447\n";
    print "$two\_vs_cpsK.bam\n";
    my $path=`ls -sh $two\_vs_cpsK.vcf`;
    print $path,"\n";
    print "breakpoint 447\n";
